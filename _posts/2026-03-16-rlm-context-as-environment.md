...
В стандартном агентном скаффолде — Claude Code, Codex, любой другой — результат каждого субвызова **дописывается токенами в контекст оркестратора**. Субагент прочитал документ и вернул резюме — это резюме теперь в истории. Следующий вызов — ещё. Контекст оркестратора растёт с каждым шагом. Сам документ тоже читается через инструменты — `read_file` возвращает текст в историю.
...
**Промежуточный контекст субвызовов выбрасывается.** Субагент думал 30К токенов — оркестратор видит только компактный return. В обычном агенте весь ход мышления оседает в истории.
...
Итог: оркестратор Claude Code при обработке большого документа упрётся в своё контекстное окно. Оркестратор RLM при 10M токенах держит в контексте только текущий код плюс компактные результаты завершённых субвызовов. Контекст не растёт пропорционально размеру входа.
