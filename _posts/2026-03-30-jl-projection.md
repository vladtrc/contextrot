...
![Куб в удобном ракурсе (поворот по x: 28°, по y: -22°, по z: 18°) с вершинами A-H и их проекциями A'-H'](/assets/jl_projection/projection_3d_to_2d.png)
...
![Расчёт расстояний CE и C'E' до и после проекции](/assets/jl_projection/cube_projection_distances.png)
...
![Матрица расстояний между вершинами куба: в ячейках показано, как расстояние до проекции отличается от расстояния после, а цвет плавно меняется от зелёного при малом искажении к красному при большом](/assets/jl_projection/cube_distance_shift.png)
...
![Куб: 3D -> 2D: точки показывают попарные расстояния до и после проекции, подпись внутри графика показывает среднее относительное искажение (20.8%)](/assets/jl_projection/cube_mean_distortion.png)
...
![Два маленьких точечных графика рядом для случайных точек при проекциях 4D -> 3D и 5D -> 4D; среднее искажение в этих прогонах равно 37.7% и 20.2%](/assets/jl_projection/random_4d_5d_side_by_side.png)
...
![График среднего относительного искажения при случайной проекции из D в D-1: по мере роста исходной размерности потеря одной координаты в среднем вредит меньше](/assets/jl_projection/distortion_vs_target_dim.png)
...
![ε-коридор для расстояния после проекции](/assets/jl_projection/eps_corridor_formula.png)
...
![Точечный график попарных расстояний до и после случайной проекции 32D -> 16D: оранжевый клин показывает ε-коридор шириной 25% вокруг диагонали, внутри него осталось 172 из 190 пар точек](/assets/jl_projection/epsilon_corridor_scatter.png)
...
![Формулировка леммы Джонсона и Линденштрауса](/assets/jl_projection/jl_lemma_formula.png)
...
![Оценка на достаточную размерность k](/assets/jl_projection/jl_dimension_bound_formula.png)
